...
デモシナリオ:
  ...
  core-link-down  : 10台構成 — CML コアリンク停止 (l0) → 5 エージェント協調診断
  node-failure    : 10台構成 — CML ディストリビューションルータ停止 (R4) → 孤立アクセス層 + 4方向クロスチェック
...
async def run_demo(scenario_name: str, scripted: bool = False) -> None:
    if scenario_name == "attack-dual":
        ...
        return
    if scenario_name in ("core-link-down", "node-failure"):
