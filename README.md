...
### 主要経路である R1-R2 間のリンク切断による冗長経路検証 (R1 <-> R2) — ✅ PASS

**障害中テスト結果**

| テスト名 | 結果 | 詳細 |
|---|---|---|
| OSPF ネイバー数確認: R1 （障害中の期待値: 1） | ❌ FAIL | 2 neighbor(s) FULL (expected: 1) |
| OSPF ネイバー数確認: R2 （障害中の期待値: 1） | ✅ PASS | 1 neighbor(s) FULL (expected: 1) |
| 確認：R3 の OSPFネイバーが 2 台（R1, R2）FULL 状態であることを確認する。 | ✅ PASS | 2 neighbor(s) FULL |
| 検証：R1からR2のLoopbackアドレス2.2.2.2へのping疎通を確認する。 | ✅ PASS | ping 2.2.2.2 OK |
| 検証：R1からR3のLoopbackアドレス3.3.3.3へのping疎通を確認する。 | ✅ PASS | ping 3.3.3.3 OK |
| 検証：R2からR3のLoopbackアドレス3.3.3.3へのping疎通を確認する。 | ✅ PASS | ping 3.3.3.3 OK |

**復旧後テスト結果**

| テスト名 | 結果 | 詳細 |
|---|---|---|
| 確認：R1 の OSPFネイバーが 2 台（R2, R3）FULL 状態であることを確認する。 | ✅ PASS | 2 neighbor(s) FULL |
| 確認：R2 の OSPFネイバーが 2 台（R1, R3）FULL 状態であることを確認する。 | ✅ PASS | 2 neighbor(s) FULL |
| 確認：R3 の OSPFネイバーが 2 台（R1, R2）FULL 状態であることを確認する。 | ✅ PASS | 2 neighbor(s) FULL |
| 検証：R1からR2のLoopbackアドレス2.2.2.2へのping疎通を確認する。 | ✅ PASS | ping 2.2.2.2 OK |
| 検証：R1からR3のLoopbackアドレス3.3.3.3へのping疎通を確認する。 | ✅ PASS | ping 3.3.3.3 OK |
| 検証：R2からR3のLoopbackアドレス3.3.3.3へのping疎通を確認する。 | ✅ PASS | ping 3.3.3.3 OK |

### R2 からのトラフィック迂回を確認 (R2 <-> R3) — ✅ PASS
