...
Cisco CML環境における設計・デプロイ・検証・デバッグのサイクルをAIエージェントが自律的に実行するシステムです。

詳細な要件定義は [instruction.md](./instruction.md) を参照してください。
...
**目標**: リンクダウン・ノード障害後の収束確認を自動化します。

`set_link_state()` は `cml_tools.py` に実装済みのため、検証エージェントのテスト計画に組み込むだけです。
...
【要件】
  ...
  3台のルーター（R1・R2・R3）をフルメッシュ（三角形）で接続する。
  各リンクで OSPF エリア 0 を構成し、R1–R2 リンクが断した場合でも
  R1→R3→R2 の迂回経路でトラフィックが継続するように設計すること。
...
[第1回 / 上限5回]  検証エージェント  開始
  [1/4] CML にデプロイ中...
    ...
    ラボを起動中...
    ノードの起動を待機中...
    起動完了 (lab_id=71586023-57f5-4a7b-8001-bc8e24f1c98b)
  [1/4] デプロイ完了 (lab_id=71586023-57f5-4a7b-8001-bc8e24f1c98b)
  [2/4] テスト計画を立案中...
  [2/4] テスト計画完了 (6 件)
  [3/4] テストを実行中...
        (1/6) Verify R1 has OSPF neighbors with R2 and R3, totaling 2 neighbors.
               → ✅ PASS  2 neighbor(s) FULL
        (2/6) Verify R2 has OSPF neighbors with R1 and R3, totaling 2 neighbors.
               → ✅ PASS  2 neighbor(s) FULL
        (3/6) Verify R3 has OSPF neighbors with R1 and R2, totaling 2 neighbors.
               → ✅ PASS  2 neighbor(s) FULL
        (4/6) Verify R1 can ping R2's Loopback0 IP address 2.2.2.2.
               → ✅ PASS  ping 2.2.2.2 OK
        (5/6) Verify R1 can ping R3's Loopback0 IP address 3.3.3.3.
               → ✅ PASS  ping 3.3.3.3 OK
        (6/6) Verify R2 can ping R3's Loopback0 IP address 3.3.3.3.
               → ✅ PASS  ping 3.3.3.3 OK
  [4/4] 全テスト PASS
...
============================================================
[Phase B]  障害シミュレーション  開始
============================================================
  [障害シミュレーション 1/3] CML からリンク一覧を取得中...
  [障害シミュレーション 2/3] 障害シナリオを LLM に立案させています (3 リンク)...
  [障害シミュレーション 2/3] 計画完了 (3 シナリオ): 各リンクはOSPFエリア0に属しフルメッシュ構成の一部であり、冗長性とフェイルオーバーによる迂回経路の実現を検証する上で重要。R1–R2のリンク断ではR1–R3–R2の迂回経路、他のリンク断ではOSPFネットワーク全体の復旧能力を確認することが目的。
  [障害シミュレーション 3/3] 障害シナリオを実行中...

  ▶ シナリオ 1/3: R1–R2間リンク断時の冗長パス確認
    リンク DOWN: R1 <-> R2 (60s 待機中...)
    テスト実行（障害中）:
        (1/6) [障害中] Verify R1 has OSPF neighbors with R2 and R3, totaling 2 neighbors.
               → ✅ PASS  1 neighbor(s) FULL
        (2/6) [障害中] Verify R2 has OSPF neighbors with R1 and R3, totaling 2 neighbors.
               → ✅ PASS  1 neighbor(s) FULL
        (3/6) [障害中] Verify R3 has OSPF neighbors with R1 and R2, totaling 2 neighbors.
               → ✅ PASS  2 neighbor(s) FULL
        (4/6) [障害中] Verify R1 can ping R2's Loopback0 IP address 2.2.2.2.
               → ✅ PASS  ping 2.2.2.2 OK
        (5/6) [障害中] Verify R1 can ping R3's Loopback0 IP address 3.3.3.3.
               → ✅ PASS  ping 3.3.3.3 OK
        (6/6) [障害中] Verify R2 can ping R3's Loopback0 IP address 3.3.3.3.
               → ✅ PASS  ping 3.3.3.3 OK
    リンク UP（復旧）: R1 <-> R2 (60s 待機中...)
    テスト実行（復旧後）:
        (1/6) [復旧後] Verify R1 has OSPF neighbors with R2 and R3, totaling 2 neighbors.
               → ✅ PASS  2 neighbor(s) FULL
        (2/6) [復旧後] Verify R2 has OSPF neighbors with R1 and R3, totaling 2 neighbors.
               → ✅ PASS  2 neighbor(s) FULL
        (3/6) [復旧後] Verify R3 has OSPF neighbors with R1 and R2, totaling 2 neighbors.
               → ✅ PASS  2 neighbor(s) FULL
        (4/6) [復旧後] Verify R1 can ping R2's Loopback0 IP address 2.2.2.2.
               → ✅ PASS  ping 2.2.2.2 OK
        (5/6) [復旧後] Verify R1 can ping R3's Loopback0 IP address 3.3.3.3.
               → ✅ PASS  ping 3.3.3.3 OK
        (6/6) [復旧後] Verify R2 can ping R3's Loopback0 IP address 3.3.3.3.
               → ✅ PASS  ping 3.3.3.3 OK
    シナリオ結果: ✅ PASS

  ▶ シナリオ 2/3: R1–R3間リンク断時における迂回経路確認
    リンク DOWN: R1 <-> R3 (60s 待機中...)
    テスト実行（障害中）:
        (1/6) [障害中] Verify R1 has OSPF neighbors with R2 and R3, totaling 2 neighbors.
               → ✅ PASS  1 neighbor(s) FULL
        (2/6) [障害中] Verify R2 has OSPF neighbors with R1 and R3, totaling 2 neighbors.
  ...
  ▶ シナリオ 3/3: R2–R3間リンク断時の復旧機能確認
    リンク DOWN: R2 <-> R3 (60s 待機中...)
    テスト実行（障害中）:
        (1/6) [障害中] Verify R1 has OSPF neighbors with R2 and R3, totaling 2 neighbors.
               → ✅ PASS  2 neighbor(s) FULL
        (2/6) [障害中] Verify R2 has OSPF neighbors with R1 and R3, totaling 2 neighbors.
...
- PASSテスト: 6 件
- FAILテスト: 0 件
- ラボID: 71586023-57f5-4a7b-8001-bc8e24f1c98b
...
lab:
  ...
  description: "3 routers in a full mesh topology with OSPF area 0."
  notes: ""
  timestamp: 0
  version: "0.1.0"
...
| テスト名 | 結果 | 詳細 |
|---|---|---|
| Verify R1 has OSPF neighbors with R2 and R3, totaling 2 neighbors. | ✅ PASS | 2 neighbor(s) FULL |
| Verify R2 has OSPF neighbors with R1 and R3, totaling 2 neighbors. | ✅ PASS | 2 neighbor(s) FULL |
| Verify R3 has OSPF neighbors with R1 and R2, totaling 2 neighbors. | ✅ PASS | 2 neighbor(s) FULL |
| Verify R1 can ping R2's Loopback0 IP address 2.2.2.2. | ✅ PASS | ping 2.2.2.2 OK |
| Verify R1 can ping R3's Loopback0 IP address 3.3.3.3. | ✅ PASS | ping 3.3.3.3 OK |
| Verify R2 can ping R3's Loopback0 IP address 3.3.3.3. | ✅ PASS | ping 3.3.3.3 OK |
...
### R1–R2間リンク断時の冗長パス確認 (R1 <-> R2) — ✅ PASS

**障害中テスト結果**

| テスト名 | 結果 | 詳細 |
|---|---|---|
| Verify R1 has OSPF neighbors with R2 and R3, totaling 2 neighbors. | ✅ PASS | 1 neighbor(s) FULL |
| Verify R2 has OSPF neighbors with R1 and R3, totaling 2 neighbors. | ✅ PASS | 1 neighbor(s) FULL |
| Verify R3 has OSPF neighbors with R1 and R2, totaling 2 neighbors. | ✅ PASS | 2 neighbor(s) FULL |
| Verify R1 can ping R2's Loopback0 IP address 2.2.2.2. | ✅ PASS | ping 2.2.2.2 OK |
| Verify R1 can ping R3's Loopback0 IP address 3.3.3.3. | ✅ PASS | ping 3.3.3.3 OK |
| Verify R2 can ping R3's Loopback0 IP address 3.3.3.3. | ✅ PASS | ping 3.3.3.3 OK |
...
### R1–R3間リンク断時における迂回経路確認 (R1 <-> R3) — ✅ PASS
...
| Verify R1 has OSPF neighbors with R2 and R3, totaling 2 neighbors. | ✅ PASS | 1 neighbor(s) FULL |
| Verify R2 has OSPF neighbors with R1 and R3, totaling 2 neighbors. | ✅ PASS | 2 neighbor(s) FULL |
| Verify R3 has OSPF neighbors with R1 and R2, totaling 2 neighbors. | ✅ PASS | 1 neighbor(s) FULL |
| Verify R1 can ping R2's Loopback0 IP address 2.2.2.2. | ✅ PASS | ping 2.2.2.2 OK |
| Verify R1 can ping R3's Loopback0 IP address 3.3.3.3. | ✅ PASS | ping 3.3.3.3 OK |
| Verify R2 can ping R3's Loopback0 IP address 3.3.3.3. | ✅ PASS | ping 3.3.3.3 OK |
