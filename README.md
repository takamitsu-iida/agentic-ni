...
│           ├── demo/                      # demo セット（R1-R2 OSPF+iBGP）
│           │   ├── requirement.md
│           │   ├── architect.md
│           │   └── validator.md
│           ├── demo2/                     # demo2 セット（R1-R2-R3 フルメッシュ・障害シミュレーション用）
│           │   ├── requirement.md
│           │   ├── architect.md
│           │   └── validator.md
...
```
iida@s400win:~/git/agentic-ni$ agentic-ni demo --troubleshoot --issue "OSPFネイバーが上がらない"
プロンプトセット: demo
トラブルシューティングモード: ラボ自動検索
問題説明: OSPFネイバーが上がらない
...
ラボを自動検出: agentic-ni-demo (ID=b5cd277e-9bca-4f89-ae0c-a1e3a7dbb058)
...
[トラブルシューティング 診断 1/3] 根本原因を分析中...
  診断: [config_error] OSPFネイバーがEXSTART状態で止まっている原因は、MTUミスの可能性が高い。R1がMTU 1452に設定されているのに対し、R2のMTUはデフォルトの1500です。

[トラブルシューティング] 修正コマンドを生成・投入中...
  修正計画: 1 件 — The OSPF adjacency is stuck in the EXSTART state due to an MTU mismatch between R1 and R2 on their GigabitEthernet0/0 interfaces. R1 is configured with an MTU size of 1452. To resolve the issue, the MTU on R2 needs to be configured to match R1's MTU size. This should enable the OSPF adjacency to progress to the FULL state.
    適用中 [R2]: Adjust MTU size on R2's GigabitEthernet0/0 interface to match R1
    ✅ 適用成功: R2
...
Traceback (most recent call last):
  File "/home/iida/git/agentic-ni/.venv/bin/agentic-ni", line 10, in <module>
    sys.exit(main())
             ~~~~^^
  File "/home/iida/git/agentic-ni/src/agentic_ni/graph.py", line 815, in main
    result = app.invoke(
  ...
  File "/home/iida/git/agentic-ni/src/agentic_ni/graph.py", line 446, in troubleshoot_verify_node
    llm = get_llm()
          ^^^^^^^
NameError: name 'get_llm' is not defined
During task with name 'verify' and id 'a854c6f3-94f3-acc1-2325-0a556664dbdf'
